...
def get_metrics(predict, target, threshold=None, predict_b=None):
    ...
    # blurred_image = cv2.GaussianBlur(np.uint8(predict*255), (5, 5), 0)
    # _, otsu_result = cv2.threshold(blurred_image,       # Input image
    #                                         0,                       # Initial threshold (ignored when using Otsu)
    #                                         255,                     # Max value after thresholding
    #                                         cv2.THRESH_BINARY + cv2.THRESH_OTSU  # Apply Otsu's method
    #                                         )
