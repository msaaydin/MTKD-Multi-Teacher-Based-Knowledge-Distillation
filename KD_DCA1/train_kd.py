...
def train_with_kd(student_model, teacher_models, trainloader, valid_loader, loss_fn, optimizer, num_epochs=5, temperature=3.0, alpha=0.7, device=None, loss_name='', scheduler = None):
    ...
    for epoch in range(num_epochs):
        ...
        if valid_loss < best_valid_loss:
            ...
            torch.save(student_model.state_dict(), f"files/T123_UNET_DistillationToStudent_DCA1.pth")
...
if __name__ == "__main__":
    ...
    teacher1.load_state_dict(torch.load("files/T1_DCA1_original.pth", map_location=device))
    teacher1.eval()

    teacher2 = build_unet()
    teacher2 = teacher2.to(device)
    teacher2.load_state_dict(torch.load("files/T2_DCA1_thick.pth", map_location=device))
    teacher2.eval()

    teacher3 = build_unet()
    teacher3 = teacher3.to(device)
    teacher3.load_state_dict(torch.load("files/T3_DCA1_thin.pth", map_location=device))
    teacher3.eval()
    ...
    train_input_dir = 'C:/Users\FSM/Desktop/MUSA/KD_DCA1/data/input/train'
    train_target_dir = 'C:/Users/FSM/Desktop/MUSA/KD_DCA1/data/target/train/orj'
                             
    val_input_dir = 'C:/Users\FSM/Desktop/MUSA/KD_DCA1/data/input/val'
    val_target_dir = 'C:/Users/FSM/Desktop/MUSA/KD_DCA1/data/target/val/orj'

    """ Dataset and loader """
    train_dataset = MultiOutputImageDataset(train_input_dir, train_target_dir, transform=transform)
    valid_dataset = MultiOutputImageDataset(val_input_dir, val_target_dir, transform=transform)
