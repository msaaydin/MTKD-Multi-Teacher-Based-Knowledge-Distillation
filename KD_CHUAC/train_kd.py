...
def distillation_loss(y_student, y_teacher, T):
    ...
    # Avoid log(0) by clamping probabilities to a small value
    eps = 1e-7
...
def train_with_kd(student_model, teacher_models, trainloader, valid_loader, loss_fn, optimizer, num_epochs=5, temperature=3.0, alpha=0.7, device=None, loss_name='', scheduler = None):
    ...
    for epoch in range(num_epochs):
        ...
        if valid_loss < best_valid_loss:
            ...
            torch.save(student_model.state_dict(), f"files/ablation/TeacherModelSeparateTest/T2S_UNET_newKD_LP_1_5_T2_to_SDistillationToStudent_CHUAC.pth")
...
if __name__ == "__main__":
    ...
    teacher1.load_state_dict(torch.load("files/T1_original_splitVal_FocalTverskyLoss.pth", map_location=device))
    teacher1.eval()

    teacher2 = build_unet()
    teacher2 = teacher2.to(device)
    teacher2.load_state_dict(torch.load("files/T2_thick_splitVal_FocalTverskyLoss.pth", map_location=device))
    teacher2.eval()

    teacher3 = build_unet()
    teacher3 = teacher3.to(device)
    teacher3.load_state_dict(torch.load("files/T3_thin_splitVal_FocalTverskyLoss.pth", map_location=device))
    teacher3.eval()                            
