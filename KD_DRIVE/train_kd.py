...
class MyDataSet(Dataset):
    def __init__(self, input_dir, target_dir, transform=None, type='orj'):
        ...
        self.input_images = glob( input_dir + "/*.tif", recursive=True)
    ...
    def __getitem__(self, idx):
        ...
        target_image_path = os.path.join(self.target_path,str.replace(os.path.basename(self.input_images[idx]), '_training','_manual1'))
        ...
        input_image = TF.pad(input_image, (0, 0, 11, 8))
        out_im = TF.pad(out_im, (0, 0, 11, 8))
...
def train_with_kd(student_model, teacher_models, trainloader, valid_loader, loss_fn, optimizer, num_epochs=5, temperature=3.0, alpha=0.7, device=None, loss_name='', scheduler = None):
    ...
    for epoch in range(num_epochs):
        ...
        if valid_loss < best_valid_loss:
            ...
            torch.save(student_model.state_dict(), f"files/T123_UNET_DistillationToStudent_DRIVE.pth")
...
if __name__ == "__main__":
    ...
    teacher1.load_state_dict(torch.load("files/T1_DRIVE_orj.pth", map_location=device))
    teacher1.eval()

    teacher2 = build_unet()
    teacher2 = teacher2.to(device)
    teacher2.load_state_dict(torch.load("files/T2_DRIVE_thick.pth", map_location=device))
    teacher2.eval()

    teacher3 = build_unet()
    teacher3 = teacher3.to(device)
    teacher3.load_state_dict(torch.load("files/T3_DRIVE_thin.pth", map_location=device))
    teacher3.eval()
    ...
    train_input_dir = 'C:/Users/FSM/Desktop/MUSA/KD_DRIVE/data/input/train'
    train_target_dir = 'C:/Users/FSM/Desktop/MUSA/KD_DRIVE/data/target/train/orj'
                             
    val_input_dir = 'C:/Users/FSM/Desktop/MUSA/KD_DRIVE/data/input/val'
    val_target_dir = 'C:/Users/FSM/Desktop/MUSA/KD_DRIVE/data/target/val/orj'

    """ Dataset and loader """
    train_dataset = MyDataSet(train_input_dir, train_target_dir, transform=transform)
    valid_dataset = MyDataSet(val_input_dir, val_target_dir, transform=transform)
   ...
   # Training loop for each loss function
    for loss_name, loss_fn in loss_functions.items():
        ...
        optimizer = optim.Adam(student_model.parameters(), lr=lr, weight_decay=1e-4)
