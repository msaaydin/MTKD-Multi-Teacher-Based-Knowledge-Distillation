...
from utils_folder.helpers import dir_exists, remove_files, double_threshold_iteration
from utils_folder.metrics import AverageMeter, get_metrics, get_metrics, count_connect_component


class Tester():
    def __init__(self, model, test_loader, dataset_path, path, seed, show=False,data_flag = None):
        ...
        if self.show:
            dir_exists(f"{self.path}/save_picture")
    ...
    def test(self, result_df, data_flag, encoder_name, seed):
        ...
        with torch.no_grad():
            for i, (img, gt) in enumerate(tbar):
                ...
                if self.data_flag == "DRIVE":
                    H, W = 584, 565
                elif self.dataset_path.endswith("CHASEDB1"):
                    H, W = 960, 999
                elif self.data_flag == "DCA1":
                    H, W = 300, 300
                ...
                if self.show:
                    ...
                    cv2.imwrite(
                        f"{self.path}/save_picture/img{i}.png", np.uint8(img.cpu().numpy()*255))
                    cv2.imwrite(
                        f"{self.path}/save_picture/gt{i}.png", np.uint8(gt.cpu().numpy()*255))
                    cv2.imwrite(
                        f"{self.path}/save_picture/pre{i}.png", np.uint8(predict*255))
                    cv2.imwrite(
                        f"{self.path}/save_picture/pre_b{i}.png", np.uint8(predict_b*255))
